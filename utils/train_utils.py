...
from utils.data_utils import annotation_jitter
        ...
        for anno in annos:
            try:
                if 'grayscale' in H and 'grayscale_prob' in H:
                    I = cv2.imread(anno.imageName, mode = 'RGB' if random.random() < H['grayscale_prob'] else 'L')
                    if len(I.shape) < 3:
                        I = cv2.cvtColor(I, cv2.COLOR_GRAY2RGB)
                else:
                    if len(I.shape) < 3:
                        continue
                    I = cv2.imread(anno.imageName, mode = 'RGB')
...
def add_rectangles(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, show_suppressed=True):
    ...
    all_rects = [[[] for _ in range(int(H["grid_width"]))] for _ in range(int(H["grid_height"]))]
    for n in range(int(rnn_len)):
        for y in range(int(H["grid_height"])):
            for x in range(int(H["grid_width"])):
...
def bilinear_select(H, pred_boxes, early_feat, early_feat_channels, w_offset, h_offset):
    ...
    x_offsets = []
    y_offsets = []
    for n in range(int(H['batch_size'])):
        for i in range(int(H['grid_height'])):
            for j in range(int(H['grid_width'])):
                for k in range(int(H['rnn_len'])):
                    batch_ids.append([n])
    ...
    pred_boxes_r = tf.reshape(pred_boxes, [int(outer_size * H['rnn_len']), 4])
