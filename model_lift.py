...
def rezoom(H, pred_boxes, early_feat, early_feat_channels, w_offsets, h_offsets):
    ...
    rezoom_features_r = tf.reshape(rezoom_features,
                                   [int(len(w_offsets) * len(h_offsets)),
                                    int(outer_size),
                                    int(H['rnn_len']),
                                    int(early_feat_channels)])
    rezoom_features_t = tf.transpose(rezoom_features_r, [1, 2, 0, 3])
    return tf.reshape(rezoom_features_t,
                      [int(outer_size),
                       int(H['rnn_len']),
                       int(len(w_offsets) * len(h_offsets) * early_feat_channels)])
...
class TensorBox(object):
    ...
    def build_forward(self, x, phase, reuse):
        ...
        cnn = tf.reshape(cnn,
                         [int(self.H['batch_size'] * self.H['grid_width'] * self.H['grid_height']), int(self.H['later_feat_channels'])])
        initializer = tf.random_uniform_initializer(-0.1, 0.1)
        with tf.variable_scope('decoder', reuse=reuse, initializer=initializer):
            scale_down = 0.01
            lstm_input = tf.reshape(cnn * scale_down, (int(self.H['batch_size'] * grid_size), int(self.H['later_feat_channels'])))
            if self.H['use_lstm']:
            ...
            for k in range(self.H['rnn_len']):
                ...
                pred_logits.append(tf.reshape(tf.matmul(output, conf_weights),
                                             [int(outer_size), 1, int(self.H['num_classes'])]))

            pred_boxes = tf_concat(1, pred_boxes)
            pred_logits = tf_concat(1, pred_logits)
            pred_logits_squash = tf.reshape(pred_logits,
                                            [int(outer_size * self.H['rnn_len']), int(self.H['num_classes'])])
            pred_confidences_squash = tf.nn.softmax(pred_logits_squash)
            pred_confidences = tf.reshape(pred_confidences_squash,
                                          [int(outer_size), int(self.H['rnn_len']), int(self.H['num_classes'])])
    ...
    def build_forward_backward(self, x, phase, boxes, flags):
        ...
        with tf.variable_scope('decoder', reuse={'train': None, 'test': True}[phase]):
            outer_boxes = tf.reshape(boxes, [int(outer_size), int(self.H['rnn_len']), 4])
            outer_flags = tf.cast(tf.reshape(flags, [int(outer_size), int(self.H['rnn_len'])]), 'int32')
            if self.H['use_lstm']:
                ...
                classes = tf.reshape(flags, (int(outer_size), 1))
    ...
    def eval(self, weights, test_boxes, min_conf, tau, show_suppressed, expname):
        self.H["grid_width"] = self.H["image_width"] / self.H["region_size"] ##region_size = 32, maybe = 64 or 128 useful?
        self.H["grid_height"] = self.H["image_height"] / self.H["region_size"]
        ...
        if self.H['use_rezoom']:
            ...
            pred_confidences = tf.reshape(tf.nn.softmax(tf.reshape(pred_confs_deltas, [int(grid_area * self.H['rnn_len']), 2])),
                                          [int(grid_area), int(self.H['rnn_len']), 2])
            if self.H['reregress']:
        ...
        with tf.Session() as sess:
            ...
            for i in range(12):
                ...
                orig_img = cv2.imread(img1_path,1)
                img = cv2.resize(orig_img, (self.H["image_width"], self.H["image_height"]), interpolation=cv2.INTER_CUBIC)
                feed = {x_in: img}  
